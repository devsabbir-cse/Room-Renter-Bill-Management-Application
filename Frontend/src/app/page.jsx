...
const Page = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if(roomNo === ""){
      toast.error("Please enter your Room No", { autoClose: 3000 });
    }
    if(mobileNo === ""){
      toast.error("Please enter your Mobile No", { autoClose: 3000 });
    }

    try {
      const response = await axios.post('http://localhost:8081/authUser', { roomNo, mobileNo });
      toast.success('Login successful', { autoClose: 3000 });
      localStorage.setItem('token', response.data.token);
      localStorage.setItem('userRoomNo', roomNo);
